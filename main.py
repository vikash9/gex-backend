...
def get_exhaustion(ticker: str = "GOOGL", timeframe: str = "1d"):
    ...
    try:
        # Enable auto_adjust=True to fix split adjustments and accurate spot prices
        df = yf.download(ticker_symbol, period=period, interval=interval, auto_adjust=True, progress=False)
        
        if df.empty:
            return {"error": f"No data found for {ticker_symbol}"}

        # Safely handle yfinance MultiIndex DataFrame columns
        ...
        for col in ['Open', 'High', 'Low', 'Close', 'Volume']:
            if col not in df.columns:
                return {"error": f"Missing price column '{col}' from yfinance payload"}
